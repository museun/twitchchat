...
async fn connect(
    ...
    let mut stream = twitchchat_async_std::connect_twitch().await?;
...
fn setup_idle_detection(writer: MpscWriter) -> ActivitySender {
    ...
    async_std::task::spawn(idle_detection_loop(input, tx));
    // and set up the responder loop
    async_std::task::spawn(respond_to_idle_events(writer, rx));
...
async fn start(user_config: &UserConfig, channels: Vec<String>) -> anyhow::Result<()> {
    ...
    async_std::task::spawn(do_some_stuff(writer.clone(), channels.clone()));
