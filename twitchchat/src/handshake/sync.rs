use crate::{
    commands,
    io::{BoxedRead, BoxedWrite, ReadHalf, WriteHalf},
    irc::IrcMessage,
    sync::{DecodeError, Decoder, Encoder, HandshakeError, Identity},
    twitch::UserConfig,
...
/// * Make this type from the `Read + Write` representation for that.
///     * If you only have an `Stream + Sink` look into this [`Handshake`][crate::stream::Handshake] instead.
///     * If you only have a `AsyncRead + AyncWrite` look into this [`Handshake`][crate::asynchronous::Handshake] instead.
...
{
    /// Create a new Handshake from the provided `Read + Write`
    ...
    ///
    /// [extend]: crate::sync::Decoder::extend()
    pub fn wait_until_ready(
        ...
        let (read, write) = crate::io::split(&mut self.inner);
        ...
        loop {
            let msg = match read.read_message() {
                Err(DecodeError::Io(err)) if is_blocking_error(&err) => continue,
                Err(err) => Err(err),
                Ok(ok) => Ok(ok),
            }?;
                ...
                .map_err(HandshakeError::from_check_err)?
            {
                ...
                ShouldPong(token) => write.encode(commands::pong(&token))?,
    ...
    pub fn split(self) -> (Decoder<ReadHalf<IO>>, Encoder<WriteHalf<IO>>) {
        let (read, write) = crate::io::split(self.inner);
